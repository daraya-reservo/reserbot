...
def lider_daily():
    ...
    lider_daily = excel_dailies[excel_dailies['Día'] == datetime.now(pytz.timezone('America/Santiago')).strftime('%Y-%m-%d')]['Responsable'].values
...
def lider_siguiente():
    ...
    excel_dailies = pandas.read_excel(f'{project_path}/dailies.xlsx', sheet_name='Hoja1') # obtener dataframe del archivo excel
    locale.setlocale(locale.LC_TIME, 'es_ES.UTF-8')  # para setear el lenguaje en el server
    dia_siguiente = datetime.now(pytz.timezone('America/Santiago')) + timedelta(days=1)
